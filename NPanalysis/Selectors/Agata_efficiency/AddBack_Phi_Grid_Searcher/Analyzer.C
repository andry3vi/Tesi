	...
	if(*nbParticleM2 == 0 && *nbParticleMG == 1 && *nbAdd == 1){
		...
		for(double i = dBetaRange[0]; i<= dBetaRange[1]; i+=dBetaStep){
		  ...
		  //double Beta = i+0.106;	//beta averaged fixed
			...
			TVector3 BetaVector(-1.0*Beta*sin(ThetaHeavy[0]*M_PI/180.0)*cos((PhiLab[0]+i)*M_PI/180.0),-1.0*Beta*sin(ThetaHeavy[0]*M_PI/180.0)*sin((PhiLab[0]+i)*M_PI/180.0),Beta*cos(ThetaHeavy[0]*M_PI/180.0)); //correction for beta direction from kinematic
			//TVector3 BetaVector(0,0,Beta);//heavy direction z axis

			double Egamma = (AddE[0]*m[AddId[0]]+q[AddId[0]])/1000; //MeV converted
