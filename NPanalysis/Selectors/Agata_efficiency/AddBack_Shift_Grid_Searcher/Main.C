...
std::vector<double> Scanner(string filename){
	...
	for(size_t i=0; i<BetaScan.size(); i++){
		...
		FWHM_sig.push_back(result[3]);
	        if(MEAN.back() != 0 ){
		       	Res->SetPoint(i,BetaScan[i],FWHM.back()/MEAN.back());
  ...
  TPad* upperPad = new TPad("upperPad", "upperPad", .005, .400, .995, .995);
  TPad* lowerPad = new TPad("lowerPad", "lowerPad", .005, .005, .995, .400);
  ...
  upperPad->cd();
	...
	TF1 * quadfit =  new TF1("quadfit","pol2",dbetamin-20,dbetamin+20);
