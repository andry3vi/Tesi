...
void Scanner(string filename){
	...
	for(size_t i=0; i<nbEntries; i++){
		...
		if(i==0){
			...
			for(size_t j=0; j<dBeta.GetSize(); j++){
				...
				Spectra.push_back(new TH1D(name.c_str(),name.c_str(),1000,200,1200));
			}
		}
		for(size_t j=0; j<dBeta.GetSize(); j++){
  ...
  cout<<endl<<endl<<"-------------------------------------------"<<endl;
  cout<<            "Best Resolution->"<<ResMin<<endl;
  cout<<            "At Beta        ->"<<BetaMin<<endl;
  cout<<            "At Shift       ->"<<ShiftMin<<endl;
  cout<<            "-------------------------------------------"<<endl<<endl;
  ...
  Centroid->GetXaxis()->SetTitle("#Agata shift [mm]");
	Centroid->GetYaxis()->SetTitle("#beta shift [v/c]");
     ...
     Centroid->DrawCopy("colz");
     Centroid->SetContour(1,contours);
     Centroid->Draw("cont3 same");
     Centroid->SetLineColor(kRed);
